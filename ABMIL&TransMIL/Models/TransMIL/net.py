...
class TransMILdropout(nn.Module):
    def __init__(self, n_classes, input_size, confounder_path=None):
        super(TransMILdropout, self).__init__()
        self.pos_layer = PPEG(dim=512)
        self._fc1 = nn.Sequential(
            nn.Linear(input_size, 512),
            nn.ReLU(),
            nn.Dropout(0.1)  # Add dropout layer here
        )
        self.cls_token = nn.Parameter(torch.randn(1, 1, 512))
        self.n_classes = n_classes
        self.layer1 = TransLayer(dim=512)
    ...
    def forward(self, feats):
        ...
        if self.confounder_path:
            norm = False
            if 'nn' not in self.confounder_path[0]:  # unnormalized 
                h = self.norm(h)[:, 0]
                device = h.device
                bag_q = self.confounder_W_q(h)
                conf_k = self.confounder_W_k(self.confounder_feat)
